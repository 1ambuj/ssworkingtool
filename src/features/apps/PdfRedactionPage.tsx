...
import {
  PDF_REDACTION_FEATURES,
  PDF_REDACTION_MAC_URL,
  PDF_REDACTION_RELEASES_PAGE,
  PDF_REDACTION_WINDOWS_URL,
} from '@/features/apps/pdf-redaction-downloads'
import { getCatalogApp } from '@/features/apps/catalog'
import { useAuth } from '@/features/auth/AuthContext'

type OsChoice = 'windows' | 'mac'

export function PdfRedactionPage() {
  const { user } = useAuth()
  const app = getCatalogApp('pdf-redaction')
  const allowed = user?.allowedApps.includes('pdf-redaction')
  const [os, setOs] = useState<OsChoice>('windows')
  const macReady = Boolean(PDF_REDACTION_MAC_URL)
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="font-display mt-1 text-3xl leading-[1.05] font-semibold tracking-tight text-ink sm:text-4xl md:text-5xl">
                  {app?.name ?? 'PDF Redaction'}
                </h1>
              </div>
            </div>
            <p className="mt-5 max-w-lg text-base leading-relaxed text-muted sm:text-lg">
              {app?.description ??
                'Offline Windows app to permanently redact PDFs.'}
            </p>

            <ol className="mt-10 space-y-0 border-t border-border/70">
              {PDF_REDACTION_FEATURES.map((feature, index) => (
                <li
                  key={feature}
          ...
          <div className="animate-fade-up-delay">
            <div className="overflow-hidden rounded-[1.75rem] border border-border/70 bg-panel shadow-[0_30px_80px_-48px_rgba(28,25,23,0.5)] lg:rounded-[2rem]">
              ...
              <div className="space-y-5 p-6 sm:p-8">
                ...
                {os === 'windows' ? (
                  <a
                    href={PDF_REDACTION_WINDOWS_URL}
                    target="_blank"
                ...
                <div className="flex flex-col gap-3 border-t border-border/60 pt-5 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-xs leading-relaxed text-muted">
                    After install, open{' '}
                    <span className="font-medium text-ink">PDF Redaction</span>{' '}
                    from the desktop or Start menu.
                  </p>
                  <a
                    href={PDF_REDACTION_RELEASES_PAGE}
                    target="_blank"
                    rel="noopener noreferrer"
